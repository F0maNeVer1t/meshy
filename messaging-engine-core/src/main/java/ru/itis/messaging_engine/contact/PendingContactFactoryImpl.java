...
import static ru.itis.messaging_engine.api.contact.HandshakeLinkConstants.FORMAT_VERSION;
import static ru.itis.messaging_engine.api.contact.HandshakeLinkConstants.ID_LABEL;
import static ru.itis.messaging_engine.api.contact.HandshakeLinkConstants.LINK_REGEX;
import static ru.itis.messaging_engine.api.contact.HandshakeLinkConstants.RAW_LINK_BYTES;
...
class PendingContactFactoryImpl implements PendingContactFactory {
	...
	public String createHandshakeLink(PublicKey k) {
		...
		return "meshy://" + Base32.encode(raw).toLowerCase(Locale.US);
	}
	...
	private PublicKey parseHandshakeLink(String link) throws FormatException {
		...
		// Discard 'meshy://' and anything before or after the link
