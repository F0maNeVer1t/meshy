...
import ru.itis.meshy.android.view.EmojiTextInputView.TextInputListener;
...
public class TextSendController implements TextInputListener {
	...
	protected void onSendStateChanged(SendState sendState) {
		if (sendState == SendState.SENT) {
			textInput.clearText();
		} else if (sendState == SendState.UNEXPECTED_TIMER) {
	...
	private void showTimerChangedDialog(boolean enabled) {
		...
		new AlertDialog.Builder(ctx, R.style.MeshyDialogTheme)
