...
public abstract class MeshyAdapter<T, V extends ViewHolder>
	...
	public MeshyAdapter(Context ctx, Class<T> c) {
		this.ctx = ctx;
		this.items = new SortedList<>(c, new SortedList.Callback<T>() {
			@Override
			public int compare(T item1, T item2) {
				return MeshyAdapter.this.compare(item1, item2);
			}
			...
			public boolean areContentsTheSame(T item1, T item2) {
				return MeshyAdapter.this.areContentsTheSame(item1, item2);
			}

			@Override
			public boolean areItemsTheSame(T item1, T item2) {
				return MeshyAdapter.this.areItemsTheSame(item1, item2);
			}
		});
	}
