...
import ru.itis.meshy.android.view.TextAvatarView;
...
class PendingContactViewHolder extends ViewHolder {
	...
	public void bind(PendingContactItem item) {
		...
		int color = ContextCompat
				.getColor(status.getContext(), R.color.meshy_gold_600);
		switch (item.getState()) {
			case WAITING_FOR_CONNECTION:
				color = ContextCompat.getColor(status.getContext(),
						R.color.meshy_orange_500);
			...
			case FAILED:
				color = ContextCompat
						.getColor(status.getContext(), R.color.meshy_red_500);
